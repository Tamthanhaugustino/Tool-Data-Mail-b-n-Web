...
export function listSavedLeads(userId: string): SavedLeadRecord[] {
...
export function saveLeads(userId: string, inputs: SaveLeadInput[]): SaveLeadsResult {
...
export function deleteSavedLead(userId: string, leadId: string): boolean {
  const store = getStore();
  const list = store.get(userId);
